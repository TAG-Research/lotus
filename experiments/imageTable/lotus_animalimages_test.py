...
df_images.load_images("ImagePath").sem_index("image_base64", "animal_index")
df_species.sem_index("species", "species_index")

res = df_images.sem_sim_join(df_species, left_on="image_base64", right_on="species", K=1)

res.drop('image_base64', axis=1, inplace=True)
...
# Output the res DataFrame to a JSON file
output_json_file = 'experiments/imageTable/animals_results.json'
res.to_json(output_json_file, orient='records', lines=True)

print(f"\nThe result has been saved to {output_json_file}")
...
for index, row in res.iterrows():
    ...
    row_class = "mismatch" if row['predicted'] != row['truth'] else ""
